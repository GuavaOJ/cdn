...
function getColOfRating(rating) {
	//这里是Guava的专属设置
	if(rating == 2333)
		return ColorConverter.toStr(ColorConverter.toRGB(new RGB(90, 160, 150)))	
	//
	if (rating < 1500) {
...
function getUserLink(username, rating, addSymbol) {
	...
	//
	if (addSymbol && username!='Guava' ) {
		if (rating >= 2500) {
			text += '<sup>';
			if (rating>=2500){
				text += "2"
			}
			for (var i = 2667; i <= rating; i += 167) {
				text += "3"
			}
			text += "</sup>";
		}
